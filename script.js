...
function showToast(msg, isErr=false){ let d=document.createElement('div'); d.innerText=msg; d.className=`fixed bottom-24 left-1/2 transform -translate-x-1/2 z-[500] px-5 py-2 rounded-full text-sm backdrop-blur-xl shadow-xl ${isErr?'bg-red-600/80':'bg-pink-600/80'} text-white`; document.body.appendChild(d); setTimeout(()=>d.remove(),2600); }
...
function updateProgressUI(){ let cont=document.getElementById('progressList'); if(!cont) return; cont.innerHTML=''; for(let f of filesToUpload){ let p=uploadProgress[f.name]||0; cont.innerHTML+=`<div class="progress-item"><div class="flex justify-between text-xs"><span>${f.name.substring(0,30)}</span><span>${p}%</span></div><div class="progress-bar"><div class="progress-fill" style="width:${p}%"></div></div></div>`; } }
...
dropZone.ondragover=e=>{ e.preventDefault(); dropZone.style.background='#ff007f20'; };
